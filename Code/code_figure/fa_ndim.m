...
maxiter = 1;
Nbest_pos = cell(1,maxiter);
Light_record = cell(1,maxiter);
Light_n = cell(1,maxiter);
for iter = 1:maxiter
tic
n = 25; 
MaxGeneration = 300; %跑一次20秒
alpha = 0.5; 
betamin = 0.2; 
gamma = 1;
...
nlight_record = zeros(seed_num,501);
for m = 1:seed_num
    ns = team_ns{1,m};
    Lightn = Light{1,m};
    F_self = team_F{1,m};
...
for k=1:MaxGeneration % 迭代500次
    ...
    %% Calculate the fitness function
    [F_self, SLL] = cost(num, virtual_array_iter);
...
%% Best solution
nbest_pos(m,:) = ns(1,:);
nLightbest(m) = Lightn2(1);
nlight_record(m,:) = light_record;
end
toc
Nbest_pos(iter) = num2cell(nbest_pos,[1 2]);
Light_record(iter) = num2cell(nlight_record,[1 2]);
Light_n(iter) = num2cell(nLightbest,[1 2]);
end
...
save('nlight_record.mat','Light_record'); 
save('nLightbest.mat','Light_n'); 
